...
namespace GXService.CardRecognize.Service
{
    ...
    public class CardRecognizeService : ICardsRecognizer
    {
        ...
        {
            var src = data.CardsBitmap.Deserialize() as Bitmap;
            if (src == null)
            {
            ...
            //黑色与白色互换
            ExchangeIndexColor(src, 255, 0);

            //去掉干扰线
            RemoveInterferenceLines(src, 0);

            //过滤噪音
            src = _blobsFiltering.Apply(src);

            //获取所有的块
            _blobCounter.ProcessImage(src);
            ...
            {
                //识别牌数字
                var cardNum = _cardNetworkService.ComputeNum(_resizeNumFilter.Apply(src.Clone(rect, src.PixelFormat)));
                
                //识别牌花色
                //var colorRect = rect;
                //colorRect.Offset(0, rect.Height * 3 / 2);
                //colorRect.Inflate(0, rect.Height / 3);
                //var cardColor = _cardNetworkService.ComputeColor(_resizeColorFilter.Apply(src.Clone(colorRect, src.PixelFormat)));
